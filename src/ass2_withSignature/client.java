...
class TCPClient {
    ...
    public static int userActive = 1;
...
class SendThread implements Runnable {
  ...
  byte[] privateKey;
	...
	public void run()
	{
        ...
        byte[] H_dash = crypto.encrypt_sign( privateKey , H);
...
class ReceiveThread implements Runnable {
  ...
  byte[] privateKey;
	...
	public void run()
	{
      ...
      String signature = inFromServer.readLine().split(" ",2)[1];

      String sender_publicKey = inFromServer.readLine().split(" ", 2)[1];

      byte[] signature_byte =  java.util.Base64.getDecoder().decode(signature);

      byte[] sender_publicKey_byte =  java.util.Base64.getDecoder().decode(sender_publicKey);
      byte[] decrypted_H = null;
      try{
        decrypted_H =  crypto.decrypt_sign(sender_publicKey_byte, signature_byte ) ;
      }catch(Exception e){}
      ...
      MessageDigest md = MessageDigest.getInstance("SHA-256");
      byte[] H = md.digest(message_byte);


      if( decrypted_H == null || !new String(H).equals(new String(decrypted_H)))
      {
        System.out.println("Message from a adversary");
      }
