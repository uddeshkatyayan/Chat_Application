    ...
    public void run()
    {
       try
       {
         ...
         while(true)
         {
           ...
           if(signature.split(" ").length!=2 || !(signature.split(" ",2)[0].equals("Signature")))
           {
           ...
           else
           {
               ...
               Quadruple senderSocket_fetching = server.userReceiveTable.get(username);

               String sender_publicKey = senderSocket_fetching.getValue3();
               outToClient_recipient.writeBytes("FORWARD "+username+"\n"+ signature + "\n" +"SenderPublicKey "+sender_publicKey+ "\n" +content_length +"\n\n"+ message);
